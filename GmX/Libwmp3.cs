...
namespace GmX
{
	...
	public unsafe class Libwmp3
	{

        
        const int ID3_TITLE = 1;
        const int ID3_ARTIST = 2;
        const int ID3_ALBUM = 4;
        const int ID3_GENRE = 8;
        const int ID3_COMMENT = 16;
        const int ID3_TRACKNUM = 32;
        const int ID3_YEAR = 64;



        const int TIME_FORMAT_MS = 1;
        const int TIME_FORMAT_SEC = 2;
        const int TIME_FORMAT_HMS = 4;
        const int TIME_FORMAT_BYTES = 8;

        const int SONG_BEGIN = 1;
        const int SONG_END = 2;
        const int SONG_CURRENT_FORWARD = 4;
        const int SONG_CURRENT_BACKWARD = 8;

        const int EQ_MAX_FREQ = 96000;
        ...
        // Single channel (Mono)
        const int MODE_SINGLE_CHANNEL = 1;
        // Dual channel (Stereo)
        const int MODE_DUAL_CHANNEL = 2;
        // Joint stereo (Stereo)
        const int MODE_JOINT_STEREO = 4;
        // Stereo
        const int MODE_STEREO = 8;


        // EMPHASISI MODE

        // none
        const int EMPHASIS_NONE = 1;
        // 50/15 ms
        const int EMPHASIS_50_15_US = 2;
        // CCIT J.17
        const int EMPHASIS_CCITT_J_17 = 4;
        // reserved
        const int EMPHASIS_RESERVED = 8;

        // ID3 TAG
        const int ID3_VERSION2 = 2;
        const int ID3_VERSION1 = 1;
        ...
        const uint MP3_MSG_STOP_ASYNC = 0x00000001;
        const uint MP3_MSG_PLAY_ASYNC = 0x00000002;
        const uint MP3_MSG_ENTER_LOOP_ASYNC = 0x00000004;
        const uint MP3_MSG_EXIT_LOOP_ASYNC = 0x00000008;
        const uint MP3_MSG_ENTER_FADE_OUT_ASYNC = 0x00000010;
        const uint MP3_MSG_EXIT_FADE_OUT_ASYNC = 0x00000020;
        const uint MP3_MSG_ENTER_FADE_IN_ASYNC = 0x00000040;
        const uint MP3_MSG_EXIT_FADE_IN_ASYNC = 0x00000080;
        const uint MP3_MSG_STREAM_BUFFER_DONE_ASYNC = 0x00000200;
        const uint MP3_MSG_STREAM_OUT_OF_DATA_ASYNC = 0x00000400;
        ...
        //
        //
        const uint MP3_MSG_STOP = 0x00010000;
        const uint MP3_MSG_PLAY = 0x00020000 ;
        const uint MP3_MSG_ENTER_LOOP = 0x00040000;
        const uint MP3_MSG_EXIT_LOOP = 0x00080000;
        const uint MP3_MSG_ENTER_FADE_OUT = 0x00100000;
        const uint MP3_MSG_EXIT_FADE_OUT = 0x00200000;
        const uint MP3_MSG_ENTER_FADE_IN = 0x00400000;
        const uint MP3_MSG_EXIT_FADE_IN = 0x00800000;
        const uint MP3_MSG_WAVE_BUFFER = 0x01000000;
        const uint MP3_MSG_STREAM_BUFFER_DONE = 0x02000000;
        const uint MP3_MSG_STREAM_OUT_OF_DATA = 0x04000000;

        const uint MP3_MSG_ALL = 0xFFFFFFFF;
        const uint MP3_MSG_ALL_ASYNC = 0x0000FFFF;
        const uint MP3_MSG_ALL_SYNC = 0xFFFF0000;



        // FFT window 

        const int  FFT_RECTANGULAR = 1;
        const int  FFT_HAMMING =  2;
        const int  FFT_HANN =  3;	
        const int  FFT_COSINE = 	4;
        const int FFT_LANCZOS =  5;	
        const int  FFT_BARTLETT =  6;
        const int  FFT_TRIANGULAR = 7;
        const int  FFT_GAUSS =  8;
        const int  FFT_BARTLETT_HANN = 9;
        const int  FFT_BLACKMAN =  10;
        const int  FFT_NUTTALL = 11;
        const int  FFT_BLACKMAN_HARRIS = 12;
        const int  FFT_BLACKMAN_NUTTALL = 13;
        const int  FFT_FLAT_TOP = 14;

        // 
        const int FFT_HARMONIC_NUM =  1;
        const int  FFT_HARMONIC_FREQ =  2;
        const int  FFT_LEFT_AMPLITUDE =  4;
        const int  FFT_RIGHT_AMPLITUDE =  8;
        const int  FFT_LEFT_PHASE =  16;
        const int  FFT_RIGHT_PHASE =  32;





        unsafe struct WAVEOUT_INFO
        {
            public uint ManufacturerID;	// Manufacturer identifier for the device driver for the device.
            public uint ProductID;			// Product identifier for the device.
            public uint DriverVersion;		// Version number of the device driver for the device.
            public uint Formats;			// Standard formats that are supported. Can be a combination of values.
            public uint Channels;			// Number specifying whether the device supports mono (1) or stereo (2) output.
            public uint Support;			// Optional functionality supported by the device.
            public fixed char ProductName[32];			// Product name in a null-terminated string.
        }
        ...
        struct STREAM_LOAD_INFO
        {
            ...
            public uint NumberOfMilliseconds;
        }

        struct MP3_TIME
        {
            public uint ms;				// number of milliseconds
            public uint sec;				// number of seconds 
            public uint bytes;				// number of bytes
            public uint frames;			// number of frames
            public uint hms_hour;			// HMS format: hours
            public uint hms_minute;		// HMS format:minutes
            public uint hms_second;		// HMS format:seconds
            public uint hms_millisecond;	// HMS format: milliseconds
        }

        struct MP3_STREAM_INFO
        {
            public uint nMPEGVersion;
            public uint nLayerVersion;
            public uint nChannelMode;
            public uint nEmphasis;
            public uint nSamplingRate;
            public uint nHeaderStart;
            public uint nChannelNumber;
            public uint fVBR;
        ...
        struct MP3_STATUS
        {
            public int fPlay;			// if song is playing this value is nonzero 
            public int fPause;			// if song is paused this value is nonzero 
            public int fStop;			// if song is stoped this value is nonzero 
            public int fEcho;			// if echo is enabled this value is nonzero 
            public int nSfxMode;		// return current echo mode
            public int fExternalEQ;	// if external equalizer is enabled this value is nonzero 
            public int fInternalEQ;	// if public internal equalizer is enabled this value is nonzero	
            public int fVocalCut;		// if vocal cut is enabled this value is nonzero
            public int fChannelMix;	// if channel mixing is enabled this value is nonzero
            public int fFadeIn;		// if song is in "fade in" public interval this value is nonzero
            public int fFadeOut;		// if song is in "fade out" public interval this value is nonzero
            public int fInternalVolume;// if public internal volume is enabled this value is nonzero
            public int fLoop;			// if song is in loop this value is nonzero
            public int fReverse;		// if song is in reverse mode this value is nonzero
            public int reservd1;		// reserved, this value is undefined
            public int reservd2;		// reserved, this value is undefined
            public int reservd3;		// reserved, this value is undefined
            public int reservd4;		// reserved, this value is undefined
        }
        ...
        public delegate int t_mp3_callback(uint message, uint param1, uint param2);
        ...
        [System.Runtime.InteropServices.DllImport("libwmp3.dll")]
        static extern unsafe uint Mp3_Initialize();
        ...
        //		To free class instance call Mp3_Destroy(uint objptr) function.
        ...
        [System.Runtime.InteropServices.DllImport("libwmp3.dll")]
        static extern unsafe int Mp3_Destroy(uint objptr);
        ...
        [System.Runtime.InteropServices.DllImport("libwmp3.dll")]
        static extern unsafe int GetVersion(uint objptr);
        ...
        [System.Runtime.InteropServices.DllImport("libwmp3.dll")]
        static extern unsafe int Mp3_GetError(
                                    uint objptr,	// Pointer to class instance.
                                    byte* pBuffer,	// Pointer to buffer receiving NULL terminated error message string.
    	...
    	[System.Runtime.InteropServices.DllImport("libwmp3.dll")]
		static extern unsafe int Mp3_SetCallbackFunc(
                                uint objptr,	// Pointer to class instance.
								t_mp3_callback  pCallbackFunc, // pointert to callback function
								uint nMessages		// specify callback mesages
);
        ...
        [System.Runtime.InteropServices.DllImport("libwmp3.dll")]
        static extern unsafe int Mp3_GetID3Field(
                                    uint objptr,		// Pointer to class instance.
                                    int nId3Version,	// ID3 version
                                    int nFieldID,		// field ID
                                    byte* pBuffer,		// Pointer to buffer receiving NULL terminated string.
                                    int nSize			// Size of receiving buffer
        );
        ...
        [System.Runtime.InteropServices.DllImport("libwmp3.dll")]
        static extern unsafe int Mp3_OpenFile(
                    uint objptr,					// Pointer to class instance.
                    string sFileName,			// pointer to name of the file 
        ...
        [System.Runtime.InteropServices.DllImport("libwmp3.dll")]
        static extern unsafe int Mp3_OpenFileW(
                    uint objptr,					// Pointer to class instance.
                    string sFileName,			// pointer to name of the file 
                    int nWaveBufferLengthMs,		// size of wave buffer ( milliseconds ), minimal 200
                    uint nSeekFromStart,	// number of bytes to move file pointer from file beginning	 
        ...
        [System.Runtime.InteropServices.DllImport("libwmp3.dll")]
        static extern unsafe int Mp3_OpenResource(
                    uint objptr,				// Pointer to class instance.
                    uint hModule,				// resource-module handle 
                     byte* sResName,			// pointer to resource name 
                     byte* sResType,			// pointer to resource type 
                    int nWaveBufferLengthMs,		// size of wave buffer ( milliseconds ), minimal 200
        ...
        [System.Runtime.InteropServices.DllImport("libwmp3.dll")]
        static extern unsafe int Mp3_Close(
                                    uint objptr	// Pointer to class instance.
        );
        ...
        [System.Runtime.InteropServices.DllImport("libwmp3.dll")]
        static extern unsafe int Mp3_EnumerateWaveOut(
                                                uint objptr	// Pointer to class instance.
        );
        ...
        [System.Runtime.InteropServices.DllImport("libwmp3.dll")]
        static extern unsafe int Mp3_GetWaveOutInfo(
                                    uint objptr,				// Pointer to class instance.
        ...
        //
        // const FORMAT_1M08	1		// 11.025 kHz, mono, 8-bit
        // const FORMAT_1M16	2		// 11.025 kHz, mono, 16-bit
        // const FORMAT_1S08	4		// 11.025 kHz, stereo, 8-bit
        // const FORMAT_1S16	8		// 11.025 kHz, stereo, 16-bit
        // const FORMAT_2M08	16		// 22.05 kHz, mono, 8-bit
        // const FORMAT_2M16	32		// 22.05 kHz, mono, 16-bit
        // const FORMAT_2S08	64		// 22.05 kHz, stereo, 8-bit
        // const FORMAT_2S16	128		// 22.05 kHz, stereo, 16-bit
        // const FORMAT_4M08	256		// 44.1 kHz, mono, 8-bit
        // const FORMAT_4M16	512		// 44.1 kHz, mono, 16-bit
        // const FORMAT_4S08	1024	// 44.1 kHz, stereo, 8-bit
        // const FORMAT_4S16	2048	// 44.1 kHz, stereo, 16-bit

        // WAVEOUT_INFO Support values

        // const CAPS_LRVOLUME 8			// Supports separate left and right volume control.
        // const CAPS_PITCH 1				// Supports pitch control.
        // const CAPS_PLAYBACKRATE 2		// Supports playback rate control.
        // const CAPS_SYNC 16				// The driver is synchronous and will block while playing a buffer.
        // const CAPS_VOLUME	4			// Supports volume control.
        // const CAPS_SAMPLEACCURATE	32	// Returns sample-accurate position information.
        // const CAPS_DIRECTSOUND 64		// 
        ...
        [System.Runtime.InteropServices.DllImport("libwmp3.dll")]
        static extern unsafe int Mp3_SetWaveOutDevice(
                                    uint objptr,			// Pointer to class instance.
        ...
        [System.Runtime.InteropServices.DllImport("libwmp3.dll")]
        static extern unsafe int Mp3_PushDataToStream(
                    uint objptr,				// Pointer to class instance.
        ...
        [System.Runtime.InteropServices.DllImport("libwmp3.dll")]
        static extern unsafe int Mp3_IsStreamDataFree(
                    uint objptr,				// Pointer to class instance.
                     void* sMemNewData		// pointer to memory block
        );
        ...
        [System.Runtime.InteropServices.DllImport("libwmp3.dll")]
        static extern unsafe int Mp3_GetStreamLoad(
                    uint objptr,						// Pointer to class instance.
        ...
        [System.Runtime.InteropServices.DllImport("libwmp3.dll")]
        static extern unsafe int Mp3_Play(
                    uint objptr	// Pointer to class instance.
        );
        ...
        [System.Runtime.InteropServices.DllImport("libwmp3.dll")]
        static extern unsafe int Mp3_Stop(
                    uint objptr	// Pointer to class instance.
        );
        ...
        [System.Runtime.InteropServices.DllImport("libwmp3.dll")]
        static extern unsafe int Mp3_PlayLoop(
                    uint objptr,				// Pointer to class instance.
        ...
        [System.Runtime.InteropServices.DllImport("libwmp3.dll")]
        static extern unsafe int Mp3_Seek(
                    uint objptr,				// Pointer to class instance.
        ...
        [System.Runtime.InteropServices.DllImport("libwmp3.dll")]
        static extern unsafe int Mp3_Pause(
                    uint objptr		// Pointer to class instance.
        );
        ...
        [System.Runtime.InteropServices.DllImport("libwmp3.dll")]
        static extern unsafe int Mp3_Resume(
                    uint objptr	// Pointer to class instance.
        );
        ...
        [System.Runtime.InteropServices.DllImport("libwmp3.dll")]
        static extern unsafe int Mp3_GetPosition(
                    uint objptr,	// Pointer to class instance.
        ...
        [System.Runtime.InteropServices.DllImport("libwmp3.dll")]
        static extern unsafe int Mp3_InternalVolume(
                    uint objptr,	// Pointer to class instance.
        ...
        [System.Runtime.InteropServices.DllImport("libwmp3.dll")]
        static extern unsafe int Mp3_SetVolume(
                    uint objptr,				// Pointer to class instance.
        ...
        [System.Runtime.InteropServices.DllImport("libwmp3.dll")]
        static extern unsafe int SetMasterVolume(
                    uint objptr,				// Pointer to class instance.
                    uint nLeftVolume,	// left channel volume
                    uint nRightVolume	// right channel volume
        );
        ...
        [System.Runtime.InteropServices.DllImport("libwmp3.dll")]
        static extern unsafe int SetPlayerVolume(
                    uint objptr,				// Pointer to class instance.
                    uint nLeftVolume,	// left channel volume
                    uint nRightVolume	// right channel volume
        );
        ...
        [System.Runtime.InteropServices.DllImport("libwmp3.dll")]
        static extern unsafe int Mp3_GetVolume(
                    uint objptr,				// Pointer to class instance.
        ...
        [System.Runtime.InteropServices.DllImport("libwmp3.dll")]
        static extern unsafe int GetMasterVolume(
                    uint objptr,				// Pointer to class instance.
                    uint* pnLeftVolume,	// pointer to variable receiving left volume
                    uint* pnRightVolume	// pointer to variable receiving right volume
        );
        ...
        [System.Runtime.InteropServices.DllImport("libwmp3.dll")]
        static extern unsafe int GetPlayerVolume(
                    uint objptr,				// Pointer to class instance.
                    uint* pnLeftVolume,	// pointer to variable receiving left volume
                    uint* pnRightVolume	// pointer to variable receiving right volume
        );
        ...
        [System.Runtime.InteropServices.DllImport("libwmp3.dll")]
        static extern unsafe int Mp3_GetMp3Info(
                    uint objptr,			// Pointer to class instance.
        ...
        //	REMARKS:
        //	typedef struct {
        //		byte *sMPEGVersion;			// Pointer to NULL terminated string specifying MPEG version.
        //		byte *sLayerVersion;		// Pointer to NULL terminated string specifying MPEG Layer version.
        //		byte *sChannelMode;			// Pointer to NULL terminated string specifying channel mode.
        //		byte *sEmphasis;			// Pointer to NULL terminated string specifying emphasis.
        ...
        [System.Runtime.InteropServices.DllImport("libwmp3.dll")]
        static extern unsafe int Mp3_GetSongLength(
                    uint objptr,		// Pointer to class instance.
        ...
        [System.Runtime.InteropServices.DllImport("libwmp3.dll")]
        static extern unsafe int Mp3_GetBitrate(
                    uint objptr,	// Pointer to class instance.
        ...
        [System.Runtime.InteropServices.DllImport("libwmp3.dll")]
        static extern unsafe int Mp3_GetStatus(
                    uint objptr,		// Pointer to class instance.
        ...
        [System.Runtime.InteropServices.DllImport("libwmp3.dll")]
        static extern unsafe int Mp3_MixChannels(
                    uint objptr,				// Pointer to class instance.
        ...
        [System.Runtime.InteropServices.DllImport("libwmp3.dll")]
        static extern unsafe int Mp3_GetVUData(
                    uint objptr,					// Pointer to class instance.
                    uint* pnLeftChannel,	// pointer to integer receiving left channel data
                    uint* pnRightChannel	// pointer to integer receiving right channel data
        );
        ...
        [System.Runtime.InteropServices.DllImport("libwmp3.dll")]
        static extern unsafe int Mp3_VocalCut(
                    uint objptr,	// Pointer to class instance.
        ...
        [System.Runtime.InteropServices.DllImport("libwmp3.dll")]
        static extern unsafe int Mp3_FadeVolume(
                    uint objptr,		// Pointer to class instance.
        ...
        [System.Runtime.InteropServices.DllImport("libwmp3.dll")]
        static extern unsafe int Mp3_FadeVolumeEx(
                    uint objptr,		// Pointer to class instance.
                    int fMode,			// specifies fade mode ( fade in ot fade out )
                    int fFormat,		// flag specifying time format
                    MP3_TIME* pTime	// pointer to MP3_TIME structure specifying fade interval
        ...
        [System.Runtime.InteropServices.DllImport("libwmp3.dll")]
        static extern unsafe int Mp3_EnableEQ(
                    uint objptr,	// Pointer to class instance.
        ...
        [System.Runtime.InteropServices.DllImport("libwmp3.dll")]
        static extern unsafe int Mp3_CreateEqBands(
                    uint objptr,		// Pointer to class instance.
        ...
        [System.Runtime.InteropServices.DllImport("libwmp3.dll")]
        static extern unsafe int Mp3_GetEqBands(
                    uint objptr,		// Pointer to class instance.
                    int* pnFreqPoint	// pointer to array of integers
        );
        ...
        [System.Runtime.InteropServices.DllImport("libwmp3.dll")]
        static extern unsafe int Mp3_SetEQParam(
                    uint objptr,		// Pointer to class instance.
        ...
        [System.Runtime.InteropServices.DllImport("libwmp3.dll")]
        static extern unsafe int Mp3_SetEQParamEx(
                    uint objptr,		// Pointer to class instance.
                    int fExternal,		// flag specifying internal or external equalizer
                    int nPreAmpGain,	// preamp gain
                    int* pnBandGain,	// array of gain for each band
        ...
        //
        //		nPreAmpGain		
        //			Preamp gain in millidecibels. Value 0 specifies no change. Positive values boost volume,
        //			negative value drops volume. This value has effect on all bands.
        //
        //		pnBandGain		
        //			Array of gain for each band ( millidecibels ). Positive values boost volume,
        //			negative value drops volume.
        //
        //		nNumberOfBands	 
        //			Number of frequency bands. Class has by default 10 eq bands, but you can create
        ...
        [System.Runtime.InteropServices.DllImport("libwmp3.dll")]
        static extern unsafe int Mp3_GetEQParam(
                    uint objptr,		// Pointer to class instance.
                    int fExternal,		// flag specifying internal or external equalizer
        ...
        //
        //		pnPreAmpGain		
        //			Pointer to integer receiving preamp gain.
        //			If this value is NULL function returns number of equalizer bands.
        //
        //		pnBandGain		
        //			Pointer to variable receiving gain for each band.
        //			If this value is NULL function returns number of equalizer bands.
        //
        //
        //	RETURN VALUES:
        ...
        [System.Runtime.InteropServices.DllImport("libwmp3.dll")]
        static extern unsafe int Mp3_GetEQParamEx(
                    uint objptr,		// Pointer to class instance.
                    int fExternal,		// flag specifying internal or external equalizer
                    int* pnPreAmpGain,	// pointer to integer receiving preamp gain
                    int* pnBandGain		// pointer to variable receiving gain for each band
        ...
        //
        //		pnPreAmpGain		
        //			Pointer to integer receiving preamp gainin millidecibels.
        //			If this value is NULL function returns number of equalizer bands.
        //
        //		pnBandGain		
        ...
        [System.Runtime.InteropServices.DllImport("libwmp3.dll")]
        static extern unsafe int Mp3_SetSfxParam(
                    uint objptr,		// Pointer to class instance.
        ...
        [System.Runtime.InteropServices.DllImport("libwmp3.dll")]
        static extern unsafe int Mp3_GetSfxParam(
                    uint objptr,		// Pointer to class instance.
        ...
        [System.Runtime.InteropServices.DllImport("libwmp3.dll")]
        static extern unsafe int Mp3_SetSfxModeData(
                    uint objptr,			// Pointer to class instance.
        ...
        [System.Runtime.InteropServices.DllImport("libwmp3.dll")]
        static extern unsafe int Mp3_GetSfxModeData(
                    uint objptr,			// Pointer to class instance.
        ...
        [System.Runtime.InteropServices.DllImport("libwmp3.dll")]
        static extern unsafe int Mp3_ReverseMode(
                    uint objptr,	// Pointer to class instance.
        ...
        [System.Runtime.InteropServices.DllImport("libwmp3.dll")]
        static extern unsafe int  Mp3_GetFFTValues(
								uint objptr,	// Pointer to class instance.
